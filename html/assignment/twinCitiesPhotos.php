...
<html lang="en">
<head>
    <!-- http://localhost/docker-LAMP-main/html/assignment/twinCitiesPhotos.php -->
...
<script>
    ...
    fetchPhotos('Leeds', '.photo-gallery-liverpool');
    fetchPhotos('Lille', '.photo-gallery-cologne');
</script>
<!--
...
updateWeatherData($conn, 'Leeds', OPENWEATHERMAP_API_KEY);
updateWeatherData($conn, 'Lille', OPENWEATHERMAP_API_KEY);
...
$liverpoolNews = fetchNewsForCity('Leeds', NEWSAPI_API_KEY);
$cologneNews = fetchNewsForCity('Lille', NEWSAPI_API_KEY);

// Insert news data into the database
updateNewsData($conn, 'Leeds', $liverpoolNews);
updateNewsData($conn, 'Lille', $cologneNews);
